...
\author{Yves Bertot\\
{\em Joint work with}\\ Assia Mahboubi and Frédérique Guilhot}
\date{July 2010}
\begin{document}
\maketitle
\begin{frame}
...
\end{frame}
\begin{frame}
\frametitle{Geometric intution: Bernstein}
\begin{center}
\includegraphics[clip=true,scale=0.4,trim=2cm 4cm 1cm 6.3cm]{control.pdf}
\end{center}
\end{frame}
\begin{frame}
\frametitle{Geometric intution: False positive}
\begin{center}
...
\item The number of changes is larger than the number of roots
\begin{itemize}
...
\item If there is only one sign change, there exist exactly one root
\begin{itemize}
...
\item elements of this basis are proportional to inverse images of \(X^k\)

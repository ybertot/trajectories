...
the polynomial in the closed interval.  We rely on a sufficient
...
{\sf Faire un truc progressif: corps algébriquement clos, puis corps réel
clos: dispose du théorème des valeurs intermédaires, la question a décrire
c'est si les racines existent dans le corps qui fournit les coefficients.}



When describing the roots of polynomials in a rational setting, we are
faced with the problem that roots are not necessarily rational. The
...
negative above helps making sure that there are no two
roots in the area being considered.  In our constructive setting, we
have a strong property that the slope is separated from 0 by a given ratio.
...
interval. This gives us a way to establish a property that is akin to
but stronger
than uniform continuity for polynomials. Starting from an interval and

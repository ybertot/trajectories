...
the \Coq{} system\cite{coqart}, with the \ssr{} extension
...
They enjoy the existence of the {\em intermediate value
...
order theory of rational numbers. The drawback of this choice is that
we consider only polynomials with rational (and not real)
coefficients, and that we need to adjust the definition of {\em root of
a polynomial} since we do not have access to real points, even algebraic
ones. Restricting the study to rational coefficients is however a
necessary milestone on the way to constructive real coefficients. And we
...
% setoids and we express all equalities using the structural ``Leibniz equality''.
...
represented by a dependent pair of a list \C{polyseq} of type \C{(seq R)} and a
proof that the last element of this list is non zero. The type
\C{(polynomial R)} is used under the notation \L+{poly R}+. The
...
If the type \C{R} of coefficients is equipped with a decidable
...
% purpose we can simply represent a polynomial  This representation can be
...
form:
\begin{theorem}[Constructive IVT for polynomial functions]
\[
\begin{array}{l}
...
$$B_{(P_0,\dots, P_n)}(t) = (1 - t)B_{(P_0, \dots, P_{n-1})} + tB_{(P_1, \dots, P_n)}$$
...
that coincides with the polynomial's graph. This is illustrated in the Figure~\ref{control}.
\begin{figure}
\begin{center}
\begin{tabular}{ccc}
...
guaranteed to contain all the roots of a polynomial and splitting this
interval into smaller pieces until all roots have been isolated.  
...
mapping,...) under the \C{Poly} construction, especially when dealing
...
have multiplicity 1).  The easy approach is to divide by he greatest common divisor
...
termination might however require more subtantial work.

...
the polynomial has the shape \(a + X \times p'\), we construct another
...
\end{center}
\caption{Finding points where a polynomial's value is very small}
\end{figure}
...
formalization: it is compatible with constructive mathematics as it 
...
be thought of having roots inside \((0,1)\) in the usual sense, this
...
and could actually be easily generalized to any uniformly continuous function
over a bounded interval.  Still, it is fair that these three aspects
of our formalization: characterization of roots and intermediate value theorem
are only crutches designed especially for the purpose of this development.
...
Even more complex operations combining associativity and reindexing
like transforming $\sum_{i = 0}^{n + m} r_i$ into
$\sum_{i = 0}^{n} r_i + \sum_{i = 0}^{m} r_{n + i}$ is again a single
rewriting step of a generting lemma about iteration of associative
operators. This \ssr{} library on idexed iterations has been of crucial
...
consequences of the transitivity of order. The \ssr{} libraries still
...
mechanism. The \Coq{} tactics on linear arithmetic are hardcoding the
...
legal ring structure for the \C{ring} tactic. This issue should be
solved by bridging the gap between structured infered by type inference
(which is the cas of our ring structures) and the OCaml database used
by \Coq{} tactics.
...
to get a truely efficient proof producing decision procedure. This is
in fact part of the long term objective of this work, namely to
...
probably crafted to relieve the user from pedestrian steps when
possible.
...
algebraic decomposition. In the short term, we want to complete this
...
of the real world, but we have to remember with caution that nature

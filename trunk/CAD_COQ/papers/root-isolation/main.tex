...
\caption{Figure~\ref{graph-desc}.  Classes of polynomials with or without
sign change}
\end{figure}
...
constant; an operation that we will name ``Horner's scheme''.
...
Polynomials with only positive or 0 coefficients have curves which
look like the curves (3.1-a) or (3.1-b) depending on whether the first
...
change), one obtains a curve whose shape is given by (c).  From then
one, multiplying a polynomial of the form (c), (d), or (e) yields a a
polynomial of the (d) form; adding a negative constant to a polynomial
of the form (d) or (e) yields a polynomial of the form (e).
Polynomials of of the form (d) or (e) share the following
characteristic: there exists a positive value \(x\), so that the
polynomial has a negative value between 0 and \(x\), and the slope of
the of the curve is strictly positive above \(x\).  Because of the slope
condition, we can also find a point where the polynomial is positive.

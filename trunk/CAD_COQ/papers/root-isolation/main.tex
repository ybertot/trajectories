...
\begin{eqnarray*}
\lefteqn{\sum_{i=0}^n \left(l + \frac{(r-l)i}{n}\right) \binom{n}{i} \frac{(x-l)^i (r-x)^{n-1}}
{(r-l)^n}}\\
...
Roots of a polynomial with coefficients in a given field are not
...
so that they enjoy the existence of the {\em intermediate value
  theorem} for polynomial functions:
...
\(a\) and \(b\). Real algebraic numbers are an intuitionistic model of
this theory, and the intermediate value property is completely
effective for these numbers.
...
analysis. As soon as one need transcendental real numbers, like $\pi$,
this set is too small.

Of course, the full classical theory of reals provide the intermediate
value property but it is by no mean effective. And this property vanishes
as such when constructive real numbers are considered.

We propose to work with an intuitionistic reformulation of the classical
theory of polynomial function which allows to abstract from these various
flavors of real numbers. For that purpose, we only work within the
first order theory of rational numbers, which is a common core
necessarily shared by all of the previous subsets of real numbers.
Therefore, we need to reformulate the notion of root of a polynomial.

Of course, the {\em intermediate value theorem} does not hold
on rational numbers. Still, it makes sense to say that a polynomial
with rational coefficients has a single \emph{real} root in a given
interval with rational endpoints, even without defining real
points. In that case indeed the polynomial crosses the real x-axis in
exactly one point, which
can be approximated arbitrarily precisely by rational numbers.
This is what we want to make precise in the next two sections.
...
making sure that there are not two roots. in this interval.  In our setting, where the
polynomials we consider only have simple roots, we have the stronger
...
\begin{figure}
\begin{center}
\includegraphics[clip=true,trim=0cm 2cm 0cm 9cm, width=0.5\textwidth]{invert.pdf}
\end{center}
...
$b'' = b_0^{(p)} \dots b_j^{(p - j)} \dots b_p^{0}$
 ...
 output by \C{dicho}.  The green area on Figure~\ref{bern} shows which values
...
implementation of real numbers, but expressed solely in terms of natural
numbers and rational numbers.  Our intention is to provide a description
of Bernstein polynomials and their characteristics that will be usable
for any choice of real numbers, without assuming an effective
...
setoids and express all equalities using the structural ``Leibniz equality''.
...
instance implemented in \Coq{} in \cite{DBLP:conf/mkm/Cruz-FilipeGW04,
russellreals, russellcoqreals} as it
...
An alternative representation of rational numbers with
...
can at any time be seen as a list of coefficients, forgetting the
...
properly defined and specified. Lists in {\tt ssreflect} are written
as
$[:: a_1, a_2, \ldots, a_n]$.  The polynomial $X$ is represented
...
a list \C{s} that has a non-zero last element, or zero, and in that case formed
...
initial proof that \C{p} was well-formed (called \C{ns}). In the second case, we

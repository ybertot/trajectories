...
concentrating on the first and third property. These proofs will be
done in the setting of polynomials with coefficient and values in an
abstract ordered archimedean field.
...
possible from the choice of implementation of rational numbers and
...
\item If the interval is bounded, we express that the interval can be
  ...
  value is always negative (resp. positive) in Figure~\ref{bounded_decompose},
  ...
  slope in Figure~\ref{bounded_decompose}), and the third
  part \(I_3\) where the polynomial's value is always
  positive (resp. negative) in Figure \ref{bounded_decompose}).
\begin{figure}[h]
\begin{center}
...
\item If the interval is unbounded, we express that the interval can
  ...
  value is always negative (resp. positive) in
  Figure~\ref{unbounded_decompose}), and the second part (\(I_2\)
  where there is a requirement on the slope with a positive
  (resp. negative) slope  in Figure~\ref{unbounded_decompose}).
\begin{figure}[h]
\begin{center}
...
parameterizing the development and {\tt (0 <= a)} stands
for a boolean value and the type constructor
...
Given asn arbitrary non constant polynomial $p$ of degree
...
\end{lstlisting}
where \C{p.[x]} denotes the Horner evaluation of the polynomial \C{p}
at the point \C{x}.

...
Add Setoid Q Qeq Q_Setoid as QeqSetoid.
...
intros (p1, p2) (q1, q2) H (r1, r2) (s1, s2); simpl in  *.
intros; Kill_times; ring.
...
auto with zarith.
apply Zmult_le_reg_r with (Zpos s2).					
unfold Zgt in |- *; auto.
  assert
 (E2 :
  (r1 * Zpos p2  * Zpos s2)%Z = (r1 * Zpos s2 * Zpos p2)%Z);
 [ ring | rewrite E2 ].
apply Zmult_le_reg_r with (Zpos q2).
unfold Zgt in |- *; auto. 
assert
 (E3 :
  ((p1 * Zpos r2 * Zpos s2  * Zpos q2))%Z =
  ((p1 * Zpos q2) * (Zpos r2 * Zpos s2))%Z); [ ring | rewrite E3 ].
assert
 (E4 :
 (r1 * Zpos s2 * Zpos p2 * Zpos q2)%Z =
  ((Zpos p2 * Zpos q2) * (r1 * Zpos s2))%Z);
  [ring | rewrite E4].
rewrite H;rewrite H0.					     
assert
  (E5 :
   (q1 * Zpos p2 * (Zpos r2 * Zpos s2))%Z=
   ((Zpos p2 * Zpos r2) * (q1 * Zpos s2))%Z);
  [ ring | rewrite E5 ].
assert
  (E6 :
(Zpos p2 * Zpos q2 * (s1 * Zpos r2))%Z=
   ((Zpos p2 * Zpos r2) * (s1 * Zpos q2))%Z);
  [ ring | rewrite E6 ].
auto with zarith.
Qed.
...
intros (p1, p2) (q1, q2) H (r1, r2) (s1, s2); simpl in *.
intros;split;intros;apply Zgt_lt;
...
apply Zmult_gt_reg_r with (Zpos q2); auto with zarith.
assert (E1 : (p1 * Zpos r2 * Zpos q2)%Z = (p1 * Zpos q2 * Zpos r2)%Z);
 [ ring | rewrite E1; rewrite  H ].
apply Zmult_gt_reg_r with (Zpos s2); auto with zarith.
assert
 (E2 :
  (r1 * Zpos p2 * Zpos q2 * Zpos s2)%Z = (r1 * Zpos s2 * Zpos p2 * Zpos q2)%Z);
 [ ring | rewrite E2; rewrite H0 ].
assert
 (E3 :
  (q1 * Zpos p2 * Zpos r2 * Zpos s2)%Z =
  (Zpos p2 * Zpos r2 * (q1 * Zpos s2))%Z); [ ring | rewrite E3 ].
assert
 (E4 :
  (s1 * Zpos r2 * Zpos p2 * Zpos q2)%Z =
  (Zpos p2 * Zpos r2 * (s1 * Zpos q2))%Z); [ ring | rewrite E4 ].
apply Zlt_gt.
apply Zmult_gt_0_lt_compat_l; auto with zarith.
Qed.

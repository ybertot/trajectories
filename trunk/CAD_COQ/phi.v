...
intro k; case k;simpl; intros;cring.
Qed.
...
intros P Q  Heq; inversion Heq.
simpl; cring.
simpl.
generalize (gcoef_P0 x);intro.
destruct i.
rewrite (IHx (PX P2 (xI i - 1) c0) P0).
unfold P0;constructor; [cring|trivial].
apply ceq_sym;trivial.
rewrite (IHx (PX P2 (xO i - 1) c0) P0).
unfold P0;constructor; [cring|trivial].
apply ceq_sym;trivial.
rewrite <-H3; apply IHx; apply PolEq_sym;trivial.
simpl.
generalize (gcoef_P0 x);intro.
destruct i.
...
unfold P0;constructor; [cring|trivial].
rewrite <-H3; apply IHx; trivial.
simpl.
destruct i; apply IHx; rewrite H0;Pring.
simpl.
destruct j.
destruct i.
simpl Pplus at 1.
...
intro h;rewrite h;Pring.
rewrite  Pplus_Pminus1_assoc.
simpl;auto.
rewrite PX_pq_pq.
rewrite  Pplus_Pminus1_assoc;simpl;Pring.
rewrite PX_pq_pq.
rewrite Pplus_minus; auto;unfold Pminus;simpl; Pring.
destruct i;simpl;apply IHx;rewrite H0.
unfold Pminus;simpl.
rewrite Pdouble_minus_one_o_succ_eq_xI;Pring.
unfold Pminus;simpl;Pring.
unfold Pminus;simpl;Pring.
simpl.
...
unfold Pminus;simpl.
rewrite Pdouble_minus_one_o_succ_eq_xI;Pring.
rewrite  Pplus_xIPminus1_assoc;simpl;Pring.
rewrite  Pplus_Pminus1_assoc;simpl;Pring.
unfold Pminus;simpl;Pring.
simpl.
unfold Pminus;simpl.
...
rewrite H0;unfold P0; constructor; cring.
...
induction P.
intros;simpl; cring.
intros;simpl Pol_mul_Rat_aux;rewrite mkPX_PX_c; cring.
simpl;cring.
induction P.
intros;simpl;cring.
...
rewrite H0;rewrite gcoef_P0; cring.
case_c0_test  (a -- c1); [assert (h:a==c1);[ apply copp_eq;trivial|rewrite h]|idtac].
cring.
...
induction n.
intros;simpl.
destruct p; try cring.
intros.
simpl.

destruct p.
case (xI p - 1)%positive;
intros; apply gcoef_Morphism; Pring.
case (xO p - 1)%positive;
intros; apply gcoef_Morphism; Pring.

case P; intros; try cring.
Qed.
...
induction n;intros.
destruct P; destruct Q; rename c0 into c2; try (simpl;cring; fail).
simpl Pol_add;destruct (ZPminus p p0);rewrite mkPX_PX_c;simpl; cring.
...
apply cadd_ext;cring.
apply gcoef_Morphism.
simpl.
cut ((xI (p3 + p2) = xO (Pplus_carry p2 p3) - 1)%positive).
intro h1; rewrite h1; apply Eq_PX_PX; try cring; try Pring.
unfold Pminus;simpl.
...
rewrite cadd_sym;apply cadd_ext;cring;
apply gcoef_Morphism.

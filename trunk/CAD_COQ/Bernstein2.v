 ...
 *)
  ...
  (*univariate polynomials over Q*)
   ...
   Definition mult_cst := (PmulC Rat Rat_coef_set)
   Definition deriv := (Pderiv Rat Rat_cef_set).
   Definition eval := (Peval Rat Rat_coef_set).
   Definition deg_coefdom := (Pdeg_coef_dom Rat Rat_coefset). 
...
(*TODO virer les contenus constants?*)
 ...
 Fixpoint sign_eval_map(l:list Poly)(x:Rat){struct l} : list comparison := 
  match l with
    |nil => nil
...
(*coefficients of 2^p P in the bernstein basis of c,(c+d)/2 and in the
 ...
 Fixpoint root_low_bound1(P:Poly)(sum:Rat){struct P}:Rat :=
   match P with
     |Pc p => sum / p
     |PX P' i p' => 
       if (Rat_zero_test p')
	 then root_low_bound1 P' sum
	 else sum / (Rat_abs_val p')
   end.

 Definition root_low_bound (P:Poly) := root_low_bound1 P (sum_abs_val_coef P).
...
(*isolates roots of P over ]c d[ *)
   ...
   Fixpoint root_isol1(res:list (TagRoot*(list Sign)))
	 ...
	 let Vb := sign_changes (map Rat_sign blist) in
	   match Vb  with
		 ...
		 then ((Pair c d P blist), (Some Eq)::nil)::res
		   else
		     match n with
			 ...
			 let res':= root_isol1 res c  mid  b' n' in
			   if (Rat_zero_test (eval Pbar mid)) 
			     then
			       root_isol1 ((Singl mid, (Some Eq)::nil)::res')
			       mid d b'' n'
			     else
		 ...
		 |S n' => 
		   ...
		   let res':= root_isol1 res c  mid  b' n' in
		     if Rat_zero_test (eval Pbar mid) 
		       then
			 root_isol1 ((Singl mid, (Some Eq)::nil)::res') mid d b'' n'
		       else
			 root_isol1 res'  mid d b'' n'
	       end
	   end.
 ...
 Section SIGN_AT.
   ...
   (*sign of Q at an int root of P which is not root of Q
...
(* refines ]ab[ which contains a unique root of P and G=gcd P G
  to a inter which isolates for Q *)
   ...
   Fixpoint sign_at_com(a b:Rat)(P Pbar G:Poly)
	 ...
	 |S O => (Pair a b G bernG, (Some Eq) :: sign_list)
	 |S _ =>
	   let mid := (a+b)/(2#1) in
	   let Pbar_mid := (eval Pbar mid) in
	     if Rat_zero_test Pbar_mid
	       then 
		 (Singl mid, (Some Eq)::sign_list)
	       else
		 match n with
  ...
  Definition sign_at_root(n:nat)(t:TagRoot*(list Sign))(low:Sign):
    TagRoot*(list Sign):=
    let (root, sign_list) :=  t in
      match root with
	|Minf => (Minf, low::sign_list)
	|Singl r => 
...
(* find the sign col after a root, evaluating only if necessary *)
 Fixpoint fill_sign_between(b:Rat)(lsign:list Sign)(lpol:list Poly)
   ...
   match lsign,lpol with
	 ...
	 |Some z =>
	   match z with
	     |Eq => (Some (Rat_sign (eval phd b)))::(fill_sign_between b stl ptl)
			 ...
			 then
			   ...
			   let res_b'_up := (add_to_cst_list resP prev_next_sign) in
			     match (Rat_zero_test Pb'), (Rat_zero_test Pa') with
			       |true, false =>
				 res_b'_up @
				 ((Singl b', (Some Eq)::prev_next_sign)::
				   refine::
   ...
   match Pol_list with
	 ...
	 |_ =>
	   ...
	   let lowsign := Rat_sign (eval P P_low) in
	     add_roots P Pfree tl prev P_low P_up (Some lowsign) (Some upsign) n
       end
   end.

...
move: abc0; rewrite le0r=>/orP; case.
   move=>/eqP/det0_aligned; case; first by move=>abe; move:ab; rewrite abe eq_refl.
   ...
   case tlt: ((1-t) * t < 0).
      ...
      rewrite 2!mulr0 oppr0 eq_refl/==>/orP; case; last by move=>/andP[acd _]; exists a; rewrite betweenl.
      move=>/orP; case=>cdb; first by exists (conv t a b); rewrite betweenl andbT.
      by exists d; rewrite betweenr andbT.
   move=>_ _; exists (conv t a b); rewrite betweenl andbT; apply between_conv.
   by exists t; rewrite eq_refl andbT in01M_ge0 leNgt mulrC tlt.
move:ab0=> _ abc; move:absep; rewrite pmulr_rle0// =>abd.
set t := det a b d / (det d a b - det c a b).
have denom: det d a b - det c a b != 0 by rewrite 2![det _ a b]det_cyclique subr_eq0; apply/negP=>/eqP detE; move:(le_lt_trans abd abc); rewrite detE lt_irreflexive.
...
case u01: (in01 u). 
   exists (conv u a b); apply/andP; split.
      apply between_conv.
      by exists u; apply/andP; split.
   rewrite utE; apply between_conv.
   exists t; rewrite eq_refl andbT in01M_ge0 -(divff denom) /t -mulrBl mulrACA addrAC ![det _ a b]det_cyclique subrr add0r mulrN -mulNr -expr2; apply mulr_ge0.
      2: by apply sqr_ge0.
...
move:cd0; rewrite cdb0 cda0 eq_refl cd/= =>/orP; case.
   2: by move=>/andP[acd _]; exists a; rewrite betweenl.
move=>/orP; case.
   by move=>cab; exists c; rewrite betweenl andbT.
by move=>dab; exists d; rewrite betweenr andbT.
Qed.
...
have sm: forall t u, t *: (u : regular_lmodType R) = t*u by [].
move:a b c d.
suff: forall a b c d, (exists p : sensDirect.Plane R, between p a b && between p c d) -> separate a b c d.
...
apply/andP; split.
   ...
   move:u01; rewrite in01M_ge0 le0r=>/orP; case.
      rewrite mulf_eq0 subr_eq0=>/orP; case=>/eqP ue; move:detE=>/eqP.
...
case ab : (a == b)=>/=.
   by move: ab=>/eqP ab; subst b; rewrite 2!det_alternate eq_refl/= andbT; apply between_conv; exists u; apply/andP; split=>//; rewrite -pe' convmm.
apply/implyP=>/eqP/det0_aligned; case; first by move=>/eqP; rewrite ab.
move=>[t'] ce; apply/implyP=> /eqP/det0_aligned; case; first by move=>/eqP; rewrite ab.
move=>[u'] de.
...
case t0: (t' < 0).
   ...
   apply/between_depl; exists (a-b), t', u'; rewrite -ce -de 2!convrl 2!eq_refl 2!andbT nmulr_rle0//.
   move:u01 tconv=>/andP[u0]; rewrite -[u<=1]subr_ge0 le0r subr_eq0 -invr_gt0=>/orP; case.
      by move=>/eqP<-; rewrite subrr mul1r mul0r addr0=>/eqP te; move:t01=>/andP[]; rewrite te leNgt t0.
   move=>ugt0.
   have un0: (1-u)^-1 != 0 by apply/negP=>/eqP ue; move:ugt0; rewrite ue lt_irreflexive.
   ...
   rewrite -mulrN; apply mulr_ge0=>//.
   by rewrite oppr_ge0; apply ltW.
move:t0=>/negbT; rewrite -leNgt=>t0.
case u1: (1 < u').
   do 2 (apply/orP; left).
   apply/between_depl; exists (b-a), (1-t'), (1-u'); rewrite -2!convlr ce de 2!eq_refl 2!andbT.
   move:u01 tconv=>/andP; rewrite le0r=>[[/orP]]; case.
      move=>/eqP-> _; rewrite subr0 mul0r mul1r add0r=>/eqP tu'.
      by move:t01; rewrite tu'=>/andP[_]/(lt_le_trans u1); rewrite lt_irreflexive.
   move:u1; rewrite -subr_lt0=>ugt1 u0 u1; rewrite nmulr_lle0//.
   have un0: u != 0 by apply/negP=>/eqP ue; move:u0; rewrite ue lt_irreflexive.
   ...
   by apply mulr_ge0; rewrite subr_ge0=>//; rewrite -subr_le0; apply ltW.
move:u1=>/negbT; rewrite -leNgt=>u1.
apply/orP; right; apply/andP; split; apply between_conv.
   exists t'; rewrite ce eq_refl andbT; apply /andP; split=>//; exact (le_trans tu u1).
exists u'; rewrite de eq_refl andbT; apply /andP; split=>//; exact (le_trans t0 tu).
Qed.
...
apply/or_asboolP/negPn; rewrite negb_or; apply/negP=>/andP[/existsp_asboolPn [t /asboolPn]]; rewrite asbool_imply negb_imply 2!asboolb=>/andP[t01 ltab].
...
have lt: forall i : 'I_(size l), 0 <= det l`_i l`_(Zp_succ i) (conv (sup I) b a).
   ...
   move: (sup_adherent eps0 Ib)=>[t]/andP[t01]; rewrite encompass_all_index l0/= =>/forallP/(_ i); rewrite andbT is_left_oriented/oriented -det_cyclique det_conv convrl sm -opprB mulrN -(pmulr_lge0 _ abl_gt0) mulrBl -mulrA divff// mulr1 subr_ge0=>lit.
   by rewrite opprB addrCA subrr addr0=>/(le_lt_trans lit); rewrite lt_irreflexive.
have I1: sup I <= 1.
...
have: [exists i : 'I_(size l), det l`_i l`_(Zp_succ i) (conv (sup I) b a) <= 0].
   move:I1; rewrite -subr_ge0 le0r subr_eq0 subr_gt0=>/orP; case=>[/eqP<- | I1].
   ...
   have It: (((sup I)%:E : @ereal_porderType R) < t `&` 1%:E)%O.
      rewrite ltxI; apply/andP; split; last by rewrite lte_fin.
      apply ereal_meets_gt; first by apply ltey.
   ...
   have tfin: (fine (t `&` 1%:E))%:E = t `&` 1%:E.
      apply/(@fineK R)/fin_numP; split; apply/negP=>/eqP tinf.
         suff : (-oo < t `&` 1)%E by rewrite tinf lt_irreflexive.
...
move=>/det0_aligned; case.
   ...
   have ilt: forall i: 'I_(size l), ((nat_of_ord i) < (size l))%N by [].
   move=>/(_ (ilt i) (ilt (Zp_succ i))); rewrite Zp_succE.
   move:(ilt i); rewrite leq_eqVlt=>/orP; case.
...
move=>[t] tie; apply between_conv; exists t; rewrite tie eq_refl andbT.
have ils: forall i : 'I_(size l), (i < size l)%N by [].
...
apply/negPn/negP; rewrite negb_and -2!ltNge=>/orP; case.
   move=>t0.
...
by rewrite -{2}(modn_small (ils i)) -addn1 -{2}(addn0 i) eqn_modDl mod0n modn_small; last by apply ltnW.
Qed.

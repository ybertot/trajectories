...
Require Export ssralg poly polydiv zint orderedalg pol.
...
move=> x px0; case: (lerP `|x| 1)=> cx1.
  ...
  rewrite -(ler_pmul2r _ _ h1) mulr1 /C mulrA mulfV ?absr_eq0 // mul1r.
  ...
  rewrite big_ord_recr /=; apply: addr_ge0=> //; exact: absr_ge0.
case e: n=> [| m].
...
have h1 : E  m.+1 * x^+m.+1 = - \sum_(i < m.+1) E i * x^+ i.
  ...
  by rewrite e //= addrC.
case x0 : (x == 0).
  rewrite (eqP x0) absr0; apply: mulr_ge0; first by rewrite invr_ge0 /= absr_ge0.
  suff h2: forall i, (i < m.+2)%N -> 0 <= `|E i|.
    by apply: sumr_ge0 => [] [i hi] /=; exact: h2.
...
have h3 : `|\sum_(i < m.+1) E i / x ^+ (m - i) | <= \sum_(i < m.+2) `|E i|.
  apply: ler_trans (absr_sum_le _ _) _.
  apply: (@ler_trans _ (\sum_(i < m.+1) `|E i|)); last first.
    by rewrite (big_ord_recr m.+1) /= ler_addr absr_ge0.
  apply: ler_sum => [] [i lti] _ /=.
  rewrite absr_mul -{2}(mulr1 (`|E i|)).
  apply: ler_pmul2rW; rewrite ?absr_ge0 //.
  rewrite absr_inv -invr1.
    (*  apply: ltef_invpp=> /=; first exact: ltr01. *)
    (* by rewrite absr_gt0 expf_eq0 x0 andbF. *)
    (* by rewrite absf_exp; apply: expf_ge1; apply: ltrW. *)
    (* monotonie inverse *)
    admit.
rewrite mulrC.
rewrite ler_pdivl_mulr; last by rewrite absr_gt0 -e.
by apply: ler_trans h3=> /=; rewrite -absr_mul mulrC h2 absr_opp lerr.
Qed.
 
End CauchyBound.
  

Section CasteljauOnAbstractRationals.
...
Lemma translate_nth : forall (p : {poly Q}) i c, (i < size p)%N ->
  (translate_pol p c)`_i =
  \sum_(k < size p) p`_k * c^+(k - i) *+ 'C(k, i).
Proof. by move=> p i c ltis; rewrite /translate_pol coef_poly ltis. Qed.

(* First linearity lemma : translate complies with scalar product for *)
(* elements of the basis. But this is not used ...
...
Qed.
*)
Lemma translate_mulX : forall (q1 q2 : {poly Q}) c, 
  q2 != 0 -> q1 != 0 ->
...
Proof.
  ...
  apply: val_inj; apply: (@eq_from_nth _ (0 : Q)).
  ...
  rewrite sp1 => [[_|j hj]].
    ...
    rewrite coef_poly lt0n size_poly_eq0 q2n0.
  ...
  rewrite /translate_pol coef_poly sp2 hj.
  ...
  move: hj; rewrite ltnS leq_eqVlt; case/orP.
    move/eqP=> ej; rewrite ej nth_default ?size_mkseq ?leqnn ?size_poly //.
    rewrite  mulr0 add0r coef_poly -ej ltnSn; apply: congr_big => // [[k hk]] _.
  ...
  move=> hjs; rewrite !coef_poly hjs (ltn_trans (ltnSn _) hjs).
  ...
  case: (ltngtP k j) => ekj.
  - rewrite !bin_small // ?mulr0n; last by apply: ltn_trans ekj _.
...
Lemma translate_polXn : forall (c : Q) i, 
...
move=> c i; rewrite -(mulr1 'X^i); elim: i => [| i ihi].
  ...
  rewrite -polyC1 coefC eqxx; apply: val_inj => /=; rewrite polyseq_cons.
  by rewrite size_poly0 eqxx /=.
rewrite exprS -mulrA.
rewrite (translate_mulX  _ _ ihi) ?exprS // ?mulr1 -size_poly_eq0.
  by rewrite size_polyXn.
by rewrite size_factor_expr.
Qed.

Lemma translate_mulXn : forall n (q1 q2 : {poly Q}) c, q2 != 0 -> q1 != 0 ->
...
rewrite (translate_mulX _ _ h); first by rewrite mulrA -exprS.
...
Lemma translateXn_addr : forall (c1 c2 : Q) n, 
  translate_pol (translate_pol 'X^n c1) c2 = translate_pol 'X^n (c1 + c2).
Proof.
move=> c1 c2 n; apply: val_inj => /=.
apply:  (@eq_from_nth _ 0); rewrite ?size_translate_pol //.
rewrite size_polyXn => i hi. 
rewrite /translate_pol size_polyXn !coef_poly hi.
rewrite size_poly_eq ?hi; last first.
  rewrite big_ord_recr /= big1.
    by rewrite add0r coef_Xn eqxx subnn binn expr0 mulr1n mulr1 nonzero1r.
  case=> j hj _ /=; rewrite coef_Xn.
  by move: hj; rewrite ltn_neqAle; case/andP; move/negPf->; rewrite mul0r mul0rn.
apply: trans_equal (_ : 
  \sum_(k < n.+1) (\sum_(k0 < n.+1)'X^n`_k0 * c1 ^+ (k0 - k) *+ 
    'C(k0, k) * c2 ^+ (k - i) *+ 'C(k, i)) = _).
  apply: congr_big => // [[k hk]] _. 
  by rewrite coef_poly hk /= big_distrl /= -sumr_muln.
...
Lemma reciprocate_padded :  forall (i : nat) (q : seq Q),
...
Lemma expand_padded :  forall (i : nat) (q : seq Q)(c : Q) , 
...
Definition delta (i j : nat) : Q := if (i == j) then 1 else 0.
...
Definition bernp i : {poly Q} := 
  ((b - a)^-p)%:P * ('X - a%:P)^+i * (b%:P - 'X)^+(p - i) *+ 'C(p, i).

Definition relocate (q : {poly Q}) :=
  let s := size q in
  ...
  if (p.+1 < s)%N then [::]
    (* 2nd case : we complete the list coefficients of q with tail zeroes *)
      ...
      translate_pol 
      ('X ^ (p.+1 - s)%N * (reciprocate_pol 
        (expand 
          (translate_pol q (- 1)) (b - a)))) (- a).
...
rewrite ltnNge ltnS leqip translate_polXn.
have side1 : forall c : Q, c != 0 ->  ~~ ((('X + (-1)%:P) ^+ i)`_i * c ^+ i == 0).
  move=> c hc; rewrite  exprn_addl coef_sum big_ord_recl /= big1; last first.
      case=> j hj _ /=; rewrite coef_natmul -polyC_exp coef_mulC coef_Xn.
      suff -> : (i == (i - bump 0 j)%N) = false by rewrite mul0r mul0rn.
      apply: negbTE; rewrite /bump add1n neq_ltn -{4}(subn0 i).
        by rewrite ltn_sub2l ?orbT //; apply: leq_ltn_trans (leq0n j) _.
      rewrite addr0 subn0 coef_natmul -polyC_exp coef_mulC coef_Xn eqxx.
      by rewrite bin0 expr0 mulr1n mulr1 mul1r expf_neq0.
have h : forall c : Q, c != 0 -> 
  expand (('X + (-1)%:P)^+ i) c = ('X * c%:P + (-1)%:P)^+ i.
  move=> c hc; rewrite /expand size_factor_expr. 
  apply: val_inj; apply:  (@eq_from_nth _ 0).
    rewrite size_amul_expr // size_poly_eq //=; exact: side1.
  move=> j; rewrite size_poly_eq => [hj|]; last exact: side1.
  rewrite coef_poly hj !exprn_addl !coef_sum big_distrl /=; apply: congr_big=> // [k hk].
  rewrite !coef_natmul -!polyC_exp !coef_mulC mulrC mulrnAr.
  congr (_ *+ _); rewrite mulrA exprn_mull -polyC_exp coef_mulC;  congr (_ * _).
  by rewrite mulrC !coef_Xn; case ej : (j == i - k)%N;  rewrite -?(eqP ej) // !mul0r.
rewrite h // {h}.
have side2 :  forall c : Q, c != 0 ->  
  ~~ ((('X * c%:P + (-1)%:P) ^+ i)`_(i.+1 - i.+1) == 0).
  move=> c hc; rewrite exprn_addl subnn coef_sum big_ord_recr big1.
    rewrite subnn coef_natmul -polyC_exp coef_mulC expr0 coefC eqxx mul1r binn.
    by rewrite mulr1n /= add0r expf_neq0 // oppr_eq0 oner_eq0.
  case=> j hj _; rewrite coef_natmul -polyC_exp coef_mulC exprn_mull -polyC_exp.
  by rewrite coef_mulC coef_Xn /= eq_sym subn_eq0 leqNgt hj !mul0r mul0rn.
have h : forall c : Q, c != 0 -> 
  reciprocate_pol (('X * c%:P + (-1)%:P) ^+ i) = (c%:P - 'X)^+i.
  move=> c hc; rewrite /reciprocate_pol size_amul_expr //.
  apply: val_inj; apply:  (@eq_from_nth _ 0).
    rewrite size_poly_eq //=; last exact: side2.
    rewrite addrC -(mul1r 'X) -mulNr mulrC -polyC_opp size_amul_expr // oppr_eq0.
    by rewrite oner_eq0.
  move=> j; rewrite size_poly_eq => [hj|]; last exact: side2.
  rewrite coef_poly hj subSS /= !exprn_addl !coef_sum.
...
rewrite h // {h} /= subSS.
suff h : translate_pol (( (b - a)%:P - 'X) ^+ i) (- a) = (b%:P - 'X)^+ i.
  rewrite (@translate_mulXn _ _ _ _ _ _ h).
  - by rewrite polyC_opp.
...
Lemma dicho'_correct : forall (a b m : Q)(alpha := (b - m) * (b - a)^-1)
  (beta := ((m - a) * (b - a)^-1))(p : nat)(q : {poly Q})(c : nat -> Q),
  m != a ->
...
Lemma bern_rev_coef : forall (p : nat)(a b : Q)(c : nat -> Q),
...
Lemma dicho_correct : forall (a b m : Q)(alpha := (b - m) * (b - a)^-1)
  (beta := ((m - a) * (b - a)^-1))(p : nat)(q : {poly Q})(c : nat -> Q),
  m != b ->
  q = \sum_(i < p.+1)(c i)%:P * bernp a b p i ->
...
End CasteljauOnAbstractRationals.

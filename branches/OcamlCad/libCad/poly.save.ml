...
exception InvalidVar of string

type 'a poly = Pc of 'a | Px of ('a poly * int * 'a )

(* in a poly space with n variables, with base cstes num,
the cst pol c. we need an Obj.magic here ... *)
let rec ncst n c = 
  if n = - 1 then Obj.magic c
  else if n = 0 then Obj. magic (Pc c)
  else Obj.magic (Pc (ncst (n - 1) c))

(* lifts polynom p living in a world with n vars to a world with n + k vars*)
let plift p k =
  let rec liftp l =
    if l = 0 then  p
    else Obj.magic (Pc (liftp (l - 1)))
  in liftp k


(* in a poly space with n variables, with base cstes num,
the monomial Xk^i. we need an Obj.magic here ... *)
let rec nkmon n k i = 
  if k = 0 then  
    (Px (Obj.magic ((ncst n Num.num_one), i, (ncst (n - 1) Num.num_zero))))
  else  (Obj.magic plift k (nkmon (n - 1) 0 i))
...
(* Information cached during computation *)
type 'a pinfo = {
  p : 'a poly;           (* a polynomial *)
  pbar : 'a poly;        (* square free part of p *)
  dpbar : int;           (* degree of pbar *)
  lowsign : sign;        (* sign of P at -infty *)
  cstsign : sign option  (* sign of p if known to be cst *)
}

let mk_pinfo p1 p2 n ls cs =
  {p = p1; pbar = p2; dpbar = n; lowsign = ls; cstsign = cs}

(* Encoding of the real algebraic number x such that x is the unique
   root of pol in ]lb, rb[ *)
type ('a, 'b) ralg = {
  mutable lb : num;          (* left bound  *)
  mutable rb : num;          (* right bound *)
  mutable pol : 'a poly;     (* pol P st P(x) = 0 *)
  mutable polbar : 'a poly;  (* square free part of *)
  mutable pbern : 'b pinfo   (* list of pinfo for each bernstein coef
                                of pol over ]rb, lb[ *)
}

(* Real sample points are :
   - either -infty, is a least bound for the roots considered in the
   pb
   - or a rational point between two consecutive roots
   - or a (known to be) rational root 
   - or an algebraic root
*)

(* special case when pol has rational coefficients *)
type ralg1 = {
  mutable lb : num;           (* left bound  *)
  mutable rb : num;           (* right bound *)
  mutable pol : num poly;     (* pol P st P(x) = 0 *)
  mutable polbar : num poly;  (* square free part of *)
  mutable pbern : num list    (* list of bernstein coefs of pol over
                                 ]lb, rb[ *)

}

type ('a, ' b) rpoint = 
    Minf of num | Btw of num | Rroot of num | Aroot of ('a, 'b) ralg

type ' a cad = Leaf of 'a | Node of 'a cad

module type Cad =
sig
    (* type of coefficients *)
  type coef
    (* type of previous dimension cell points *)
  type path
    (* type of bernstein coefs info *)
  type berncoef

  type poly_c    = coef poly
  type next_path = path * ((coef, berncoef) ralg)
  type pi        = coef pinfo
  type cad_c     = next_path cad    
  val pzero : poly_c
  val pone : poly_c

  val ( /+  )        : poly_c -> poly_c -> poly_c
  val ( /-  )        : poly_c -> poly_c -> poly_c
  val ( /*  )        : poly_c -> poly_c -> poly_c
  val ( //  )        : poly_c -> poly_c -> poly_c
  val copp           : poly_c -> poly_c
  val ppow           : poly_c -> int -> poly_c
  val cpmul          : poly_c -> num    -> poly_c 
  val cpdiv          : poly_c -> num -> poly_c
  val p_of_c         : num -> poly_c
  val p_is_c         : poly_c -> bool
  val p_is_0         : poly_c -> bool
  val p_cst_sign     : poly_c -> sign option
  val p_gcd_gcd_free : poly_c -> poly_c -> poly_c * poly_c * poly_c
  val p_square_free  : poly_c -> poly_c
  val path_refine    : next_path -> int -> next_path option
  val plow_bound     : poly_c -> path -> num
  val p_val_bound    : next_path -> poly_c -> num * num  
  val info_of_p      : sign -> poly_c -> pi 
  val p_low_s        : path -> poly_c -> poly_c -> int -> (path *(sign option))
  val ps_at          : pi -> next_path -> int -> next_path ->
    (poly_c * sign)
  val pcad           : poly_c list -> int -> cad_c
end
...
sig
  ...
  val ( /-- )        : coef -> coef -> coef
  val ( /// )        : coef -> coef -> coef
  val czero_test     : coef -> bool
  val ( /^^ )        : coef -> int -> coef
  val c_of_int       : int -> coef  
  val c_of_num       : num -> coef
  val cis_num_test   : coef -> bool
  val string_of_coef : coef -> string
end

module PolyOps (COps : CoefOps) =
struct
  ...
  (* normalizing version of Px constructor *)
  let mkPx p i c = 
    match p with
      |Pc p' -> 
         if (czero_test p') then Pc c 
         else Px (p, i, c)
      |Px (p', i', c') ->
         if (czero_test c') then Px (p', i + i', c)
         else Px (p, i, c)

  let rec popp p =
    match p with
      |Pc c -> Pc (copp c)
      |Px (p', i, c) -> mkPx (popp p') i (copp c)
  ...
  let rec padd p1 p2 =
    match p1, p2 with
      |Pc c1', _  -> paddc p2 c1'
      |_, Pc c2' -> paddc p1 c2'
      |Px (p1', i1, c1'), Px (p2', i2, c2') ->
         if i1 = i2 then mkPx (padd p1' p2') i1 (c1' /++ c2')
         else if i1 < i2 then 
           mkPx (padd p1' (mkPx p2' (i2 - i1) c0)) i1 (c1' /++ c2')
         else
           mkPx (padd p2' (mkPx p1' (i1 - i2) c0)) i2 (c1' /++ c2')
  ...
  let rec psub p1 p2 =
    match p1, p2 with
      |_, Pc c2'  -> psubc p1 c2'
      |Pc c1', _ -> paddc (popp p2) c1'
      |Px (p1', i1, c1'), Px (p2', i2, c2') ->
         if i1 = i2 then mkPx (psub p1' p2') i1 (c1' /-- c2')
         else if i1 < i2 then 
           mkPx (psub p1' (mkPx p2' (i2 - i1) c0)) i1 (c1' /-- c2')
         else
           mkPx (psub p2' (mkPx p1' (i1 - i2) c0)) i2 (c1' /-- c2')
  ...
  let rec pmul p1 p2 =
    match p2 with
      |Pc c2' ->  pmulc p1 c2'
      |Px (p2', i, c2') -> 
         padd (mkPx (pmul p1 p2') i c0) (pmulc p1 c2')
  ...
  let ppow p i = 
    let rec ppow_aux one p i =
    match i with
      | i when i < 0 -> invalid_arg "fast_power requires non-negative n"
      | 0 -> one (* "one" is used as product accumulator *)
      | i -> 
          let sq = pmul p p in (* x contains the last square *)
            if (i mod 2) = 1 then ppow_aux (pmul one p) sq (i / 2)
            else ppow_aux one sq (i / 2)
    in
      ppow_aux p1 p i
  ...
  let rec pder p = 
    match p with
      ...
      |Px (q, i, c) ->
         match i with
           |i when i <= 0 -> invalid_arg "poly not in nf"
           |1 -> q
           |i -> padd 
              (mkPx (pder q) i c0) (pmulc (Px (q, (i - 1), c)) (c_of_int i))

  let p_of_int i = Pc (c_of_int i)

  let p_of_num n = Pc (c_of_num n)
    
  let rec peval p x =
    match p with
      |Pc c -> c
      |Px (p', i, c') ->
         if i <= 0 then invalid_arg "poly not in nf"
         else (peval p' x) /** ((x /^^ i) /++ c')

  (* list of truncations of p : next truncation is added to the list
     if the leading coef is not a num constant *)
  let ptrunk p =
    let rec ptrunk_aux p tlist clist =
      match p with
        |Pc c -> 
           if cis_num_test c then (p :: tlist, clist) 
           else (p :: p0 :: tlist, c::clist)
        |Px (q, i, c) ->
           let (tres, cres) = ptrunk_aux q tlist clist in
             (map (fun x -> mkPx x i c) tres, cres)
    in
      ptrunk_aux p [] []

  (* division of p by a coef. we are in zero characteristic. like an
     euclidean division, yet if division over coefs fails, returns 0 *)


  (* due to Coq translation we have here a strange mix of exception
     and zero default values. hopefully harmless yet to be fixed*)
  let rec pdivc p a =
    if czero_test a then invalid_arg "division by zero"
    else
      match p with
        |Pc c -> Pc (c /// a)
        |Px (q, i, c) ->
           let q' = (pdivc q a) in 
             if pzero_test q' then p0
             else if czero_test c then Px (q', i, c0)
             else let c' = c /// a in
               if czero_test c' then p0
               else Px (q', i, c')
...
module BaseCoefs_Q : CoefOps
  with type coef = Num.num
=
struct
  type coef    = Num.num
  type poly_c    = Num.num poly

  let c0           = Num.num_zero
  let c1           = Num.num_one
  let ( /++ )      = Num.add_num
  let ( /** )      = Num.mult_num
  let copp         = fun x -> Num.sub_num c0 x 
  let ( /-- )      = Num.sub_num
  let ( /// )      = Num.div_num
  let czero_test   = fun (x : Num.num) -> x = Num.num_zero
  let ( /^^ )      = fun (x : Num. num) i -> Num.power_num x (Num.num_of_int i)
  let c_of_int     = Num.num_of_int 
  let c_of_num     = fun (x : Num.num) -> x
  let cis_num_test = fun (x : Num.num) -> true
  let string_of_coef = Num.string_of_num
end



module PolyOpsQ = PolyOps(BaseCoefs_Q)

let test = PolyOpsQ.p0
